...
void
persist_state_register_dump_func(PersistState *self, const gchar *name_prefix, PersistStateDumpFunc callback)
{
  if (!dump_funcs)
    return;
...
persist_state_get_dump_func(PersistState *self, const gchar *name)
{
  if (!dump_funcs)
    return NULL;
